...
@router.get('/persons/')
async def get_persons( roll_number: str , db: Session = Depends(get_db)):
    return await service.get_persons(db , roll_number)
...
@router.post('/persons/')
async def post_persons(db: Session = Depends(get_db)):
    return await service.post_persons(db)
